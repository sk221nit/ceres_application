...
namespace ceresapp
{
    ...
    BundleAdjustment::BundleAdjustment(const std::string &filename, bool use_quaternions)
    {
    ...
    void BundleAdjustment::WriteToFile(const std::string filename) const
    {
    ...
    void BundleAdjustment::WriteToPLYFile(const std::string filename) const
    {
        ofstream outfile;
        ...
        for (int i = 0; i < num_cameras(); ++i)
        {
            ...
            outfile << center[0] << ' ' << center[1] << ' ' << center[2] << " 0 255 0"
                    << '\n';
        }
